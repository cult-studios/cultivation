...
const GLOW = { teal: TEAL, purple: PURPLE, gold: GOLD, magenta: MAGENTA, blue: "#8fb6e0", green: ENERGY };
...
function Doodle({ name, size = 26, tint = "teal", glow = false, style = {} }) {
  const f = DOODLE_FILTER[tint] || "none";
  return (
    <img src={`assets/doodles/${name}.svg`} alt="" draggable={false}
      style={{ width: size, height: size, objectFit: "contain", filter: glow ? `${f} drop-shadow(0 0 7px ${GLOW[tint] || TEAL}aa)` : f, display: "block", ...style }} />
  );
}
...
const SOCIAL = [
  ["bsky", "Bluesky"], ["discord", "Discord"], ["instagram", "Instagram"], ["youtube", "YouTube"],
];
...
function HandNote({ children, color = GOLD, size = 22, style = {} }) {
  return <span style={{ fontFamily: hand, fontSize: size, color, lineHeight: 1.1, textShadow: `0 0 14px ${color}55`, ...style }}>{children}</span>;
}

function Hero() {
  return (
    <section style={{ textAlign: "center", padding: "96px 24px 78px", position: "relative", overflow: "hidden",
      ...
      {/* floating bioluminescent doodles */}
      <div style={{ position: "absolute", left: "9%", top: "24%", opacity: 0.85, transform: "rotate(-12deg)" }}><Doodle name="sparkle" size={30} tint="gold" glow /></div>
      <div style={{ position: "absolute", right: "11%", top: "26%", opacity: 0.85, transform: "rotate(10deg)" }}><Doodle name="flower" size={34} tint="magenta" glow /></div>
      <div style={{ position: "absolute", right: "18%", bottom: "14%", opacity: 0.8 }}><Doodle name="moon" size={26} tint="purple" glow /></div>
      ...
      <div style={{ position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 14, justifyContent: "center", flexWrap: "wrap" }}>
          <a href="#neighbors" style={{ fontFamily: sans, fontWeight: 600, fontSize: 16, color: CREAM, textDecoration: "none", padding: "15px 34px", borderRadius: 999,
            background: "linear-gradient(135deg, #2f6e57, #3f9068)", border: `1px solid ${TEAL}66`, boxShadow: `0 0 26px ${TEAL}44, 0 6px 20px rgba(0,0,0,0.4)` }}>meet the neighbors</a>
        </div>
      </div>
    </section>
  );
...
function Lift({ children, pad = 24, radius = 18, glow = TEAL }) {
  ...
  return (
    <div onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} style={{
      ...
      transform: h ? "translateY(-5px)" : "none", transition: "all .3s var(--ease-out)",
    }}>{children}</div>
  );
}

function SectionHead({ kicker, title, sub, kickerColor = TEAL }) {
  return (
    <div style={{ textAlign: "center", marginBottom: 44 }}>
      {kicker && <p style={{ fontFamily: mono, fontSize: 12, letterSpacing: "0.16em", textTransform: "uppercase", color: kickerColor, margin: "0 0 11px", textShadow: `0 0 12px ${kickerColor}55` }}>{kicker}</p>}
      <h2 style={{ fontFamily: serif, fontSize: 40, fontWeight: 600, color: CREAM, margin: 0, textShadow: `0 0 24px ${kickerColor}30` }}>{title}</h2>
      {sub && <p style={{ fontFamily: sans, fontSize: 16, color: BODY, maxWidth: 564, margin: "14px auto 0", lineHeight: 1.6 }}>{sub}</p>}
    </div>
  );
}
...
function Features() {
  return (
    <section style={{ padding: "78px 24px", background: `radial-gradient(50% 60% at 12% 0%, ${TEAL}14, transparent 60%), ${NIGHT2}` }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        <SectionHead kicker="what you'll do" title="a real farm, mostly legal" />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 20 }}>
          {FEATURES.map(f => (
            <Lift key={f.title} glow={GLOW[f.tint]}>
              <div style={{ marginBottom: 14 }}><Doodle name={f.doodle} size={30} tint={f.tint} glow /></div>
              <h3 style={{ fontFamily: serif, fontSize: 22, color: CREAM, margin: "0 0 9px" }}>{f.title}</h3>
              <p style={{ fontFamily: sans, fontSize: 15, color: BODY, lineHeight: 1.6, margin: 0 }}>{f.text}</p>
            </Lift>
          ))}
        </div>
      </div>
          ...
          {CHARACTERS.map(c => (
            <Lift key={c.name} radius={18} pad={24} glow={GLOW[c.tint]}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 13 }}>
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: INK, border: `1px solid ${GLOW[c.tint]}77`, boxShadow: `0 0 16px ${GLOW[c.tint]}44, inset 0 0 12px ${GLOW[c.tint]}22`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <Doodle name={c.doodle} size={24} tint={c.tint} glow />
                </div>
                <div>
                  <h3 style={{ fontFamily: serif, fontSize: 19, color: CREAM, margin: 0, lineHeight: 1.1 }}>{c.name}</h3>
                  <div style={{ fontFamily: mono, fontSize: 10.5, letterSpacing: "0.05em", textTransform: "uppercase", color: MUTE, marginTop: 4 }}>{c.role}</div>
                </div>
              </div>
              <p style={{ fontFamily: sans, fontSize: 14.5, color: BODY, lineHeight: 1.6, margin: 0 }}>{c.text}</p>
            </Lift>
          ))}
        </div>
      </div>
...
function Journal() {
  ...
  return (
    <section style={{ padding: "78px 24px", background: `radial-gradient(45% 55% at 50% 0%, ${GOLD}12, transparent 60%), ${NIGHT2}` }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        <SectionHead kicker="field notes" kickerColor={GOLD} title="from the developer's garden" />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 20 }}>
          {posts.map(p => (
            <Lift key={p} radius={16} glow={GOLD}>
              <div style={{ display: "flex", alignItems: "center", gap: 7, fontFamily: mono, fontSize: 11, letterSpacing: "0.06em", textTransform: "uppercase", color: GOLD, marginBottom: 10 }}>
                <Doodle name="pencil" size={14} tint="gold" /> coming soon
              </div>
              <h3 style={{ fontFamily: serif, fontSize: 19, color: CREAM, margin: 0, lineHeight: 1.3 }}>{p}</h3>
            </Lift>
          ))}
        </div>
      </div>
...
function AfterDark() {
  return (
    <section style={{ padding: "84px 24px", position: "relative", overflow: "hidden",
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", position: "relative" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(290px, 1fr))", gap: 44, alignItems: "center" }}>
          ...
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ marginTop: 20 }}><HandNote color={GOLD} size={20}>(play it safe... or don't)</HandNote></p>
          </div>
        </div>
      </div>
    </section>
...
function SocialDot({ f, label }) {
  const [h, setH] = useState(false);
  return (
    <div onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} title={`${label}: coming soon`}
      style={{ position: "relative", width: 46, height: 46, borderRadius: "50%", background: PANEL,
        border: `1px solid ${h ? TEAL + "55" : LINE}`, boxShadow: h ? `0 0 14px ${TEAL}33` : "none",
        display: "flex", alignItems: "center", justifyContent: "center", cursor: "default", transition: "all .25s var(--ease-out)" }}>
      <img src={`assets/icons/social/${f}.png`} alt={label}
        style={{ width: 24, height: 24, objectFit: "contain", filter: h ? "grayscale(0.15) opacity(0.95)" : "grayscale(1) opacity(0.4)", transition: "all .25s var(--ease-out)" }} />
      <span style={{ position: "absolute", bottom: "calc(100% + 9px)", left: "50%", transform: `translateX(-50%) translateY(${h ? 0 : 4}px)`,
        opacity: h ? 1 : 0, pointerEvents: "none", whiteSpace: "nowrap",
        fontFamily: mono, fontSize: 10, letterSpacing: "0.08em", textTransform: "uppercase", color: GOLD,
        background: INK, border: `1px solid ${GOLD}55`, boxShadow: `0 0 12px ${GOLD}33`, borderRadius: 999, padding: "4px 11px",
        transition: "all .25s var(--ease-out)" }}>coming soon</span>
    </div>
  );
}
...
function Site() {
  return (
    <div data-brand="cultivation" style={{ background: INK, minHeight: "100%" }}>
      <Nav />
      <Hero />
      <Features />
      <Characters />
